...
std::vector<std::vector<uint32_t>> smoothVec;
std::vector<std::vector<std::vector<uint32_t>>> smoothFactorsVec; // Factorization of each B-smooth number.
...
void sieve(uint32_t intervalStart, uint32_t intervalEnd, std::vector<uint32_t> factorBase, std::vector<float> &logApprox, std::pair<std::vector<uint32_t>, std::vector<uint32_t> > &startIndex){
    for (uint32_t i = 0; i < factorBase.size(); ++i) {
      ...
      if (p == 2) continue;  // a^2 = N (mod 2) only has one root.
...
void threadQS(int index, mpz_class N, uint32_t intervalStart, uint32_t intervalEnd, std::vector<uint32_t> factorBase){
     ...
     * Step 2.1.1
     *
     * Generate log approximations of Q = (a + sqrt(N))^2 - N in the current
     * interval.
     */
     ...
     * Step 2.1.2
     *
     * Sieve for numbers in the sequence that factor completely over the factor
     * base.
     */
  
    // First bottleneck of this while loop
    ...
    // Second bottleneck of this while loop
    for (uint32_t i = 0, a = intervalStart; i < INTERVAL_LENGTH; ++i, ++a) {
            ...
            if (Q == 1) {
                ...
                smoothFactorsVec[index].push_back(factors);
                smoothVec[index].push_back(a);
            }
    }
}
 ...
 */
mpz_class quadraticSieve(int threadNum, const mpz_class& N) {
    ...
    // Each thread need to do each interval of QS and master thread need to check the amount of smooth
    ...
    // Here is the main bottleneck of QS when N isn't too big
    clock_t smoothtimer = clock();
    for(int i=0; i<threadNum; i++){
        std::vector<uint32_t> tmp_smooth;
        smoothVec.push_back(tmp_smooth);
        std::vector<std::vector<uint32_t>> tmp_smoothFactors;
        smoothFactorsVec.push_back(tmp_smoothFactors);
    }

    std::vector<std::thread> threads;
    while (smooth.size() < factorBase.size() + 20) {
        for(int i=0; i<threadNum; i++){
            threads.emplace_back(threadQS, i, N, intervalStart, intervalEnd, factorBase);
            // Move on to next interval.
            intervalStart += INTERVAL_LENGTH;
            intervalEnd += INTERVAL_LENGTH;
        }
        for(int i=0 ; i<threadNum; i++){
            // Join result of thread and collect result to smotth and smoothFactors
            threads[i].join();
            smooth.insert(smooth.end(), smoothVec[i].begin(), smoothVec[i].end());

            //smoothFactors
            smoothFactors.insert(smoothFactors.end(), smoothFactorsVec[i].begin(), smoothFactorsVec[i].end());

            // Reset smoothVec and smoothFactorsVec
            smoothVec[i].clear();
            smoothFactorsVec[i].clear();
        }
        for(int i=0; i<threadNum; i++){
            threads.pop_back();
        }
    }
    std::cerr << "Find enough smooth numbers (>= factorBase.size() + 20) to solve Linear Algebra" << " in ";
    std::cerr << (1000.0 * (std::clock() - smoothtimer)/CLOCKS_PER_SEC)/threadNum;
    ...
    // When smooth number get bigger, solve Linear algebra will become another bottleneck
    START();
    ...
    START();
    M.reduce();
    STOP("Solve Linear Algebra-2 Reduce");
